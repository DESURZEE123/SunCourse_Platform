...
export default () => {
  ...
  const [studentMapList, setStudentMapList] = useState([])
  useEffect(() => {
    Promise.all([
      ...
      request('/api/user/get/student', { method: 'GET' })
    ])
      .then(([departData, courseData, classData, teacherData, studentData]) => {
        ...
        setStudentMapList(createMap(studentData.data, 'stuId', 'name'));
      })
      .catch((error) => {
  ...
  return { departMapList, courseMapList, classMapList, teacherMapList, studentMapList }
}